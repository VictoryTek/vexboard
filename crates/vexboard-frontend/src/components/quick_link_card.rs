...
pub fn QuickLinkCard(
    ...
    view! {
        ...
        >
            ...
            // Edit / Delete actions — only shown to admin; stop propagation so clicks don't follow the link
            {(on_edit.is_some() || on_delete.is_some()).then(|| view! {
                ...
                >
                    {on_edit.map(|cb| view! {
                        <button
                            ...
                            on:click=move |_| cb.run(link_id)
                        >
