...
async fn fetch_quick_link_groups_internal() -> Vec<QuickLinkGroupEntry> {
    let Ok(resp) = gloo_net::http::Request::get("/api/v1/quick-link-groups")
        .send()
        .await
    else {
        return Vec::new();
    ...
    resp.json::<Vec<QuickLinkGroupEntry>>()
        .await
        .unwrap_or_default()
}
...
#[component]
pub fn QuickLinkGroupsModal(
    #[prop(into)] visible: Signal<bool>,
    #[prop(into)] on_close: Callback<()>,
    #[prop(into)] on_saved: Callback<()>,
) -> impl IntoView {
    let groups = LocalResource::new(fetch_quick_link_groups_internal);
    ...
    let do_create = move || {
        ...
        spawn_local(async move {
            ...
            if let Ok(req) = gloo_net::http::Request::post("/api/v1/quick-link-groups").json(&body)
            {
    ...
    let do_rename = move |id: i64| {
        ...
        spawn_local(async move {
            ...
            if let Ok(req) =
                gloo_net::http::Request::put(&format!("/api/v1/quick-link-groups/{id}")).json(&body)
            {
                let _ = req.send().await;
            }
            editing_id.set(None);
    ...
    let do_delete = move |id: i64| {
        let on_saved = on_saved;
        spawn_local(async move {
            let _ = gloo_net::http::Request::delete(&format!("/api/v1/quick-link-groups/{id}"))
                .send()
                .await;
            groups.refetch();
            on_saved.run(());
    ...
    let do_move = move |id: i64, direction: i32| {
        ...
        spawn_local(async move {
            ...
            if let Ok(req) =
                gloo_net::http::Request::put(&format!("/api/v1/quick-link-groups/{}", a.id))
                    .json(&body_a)
            {
                let _ = req.send().await;
            }
            if let Ok(req) =
                gloo_net::http::Request::put(&format!("/api/v1/quick-link-groups/{}", b.id))
                    .json(&body_b)
            {
                let _ = req.send().await;
            }
            ...
            <div style="position:fixed; inset:0; z-index:50; display:flex; align-items:center; justify-content:center;">
                ...
                <div style="position:relative; background:var(--color-bg-surface); border:1px solid var(--color-border); \
                    ...
                    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:1.25rem;">
                        <h2 style="font-size:1rem; font-weight:600; margin:0;">"Manage Quick Link Groups"</h2>
                        <button
